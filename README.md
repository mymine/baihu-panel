...
### 最近更新
**2026.07.20** - **Windows 平台适配与 Ctrl+C 中断**：重构并建立专有的前后端 Windows 平台包；网页终端支持 `Ctrl+C` 递归终结子进程树；xterm 终端自动换行防乱折行；Monaco 编辑器高亮 `.bat/.cmd/.ps1` 并添加保存挂起风险审查；Makefile 新增 `release-windows` 并整合 GitHub Actions 自动发布 Windows 单文件压缩包。  
